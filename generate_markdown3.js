 ...
 *  node generate_markdown3.js ${xboard_api_secret} ${xboard_api_url}
 */
const fs = require("fs");
...
function fetchSubscriptions(credentials, xboard_api_url) {
    ...
    const templateSource = fs.readFileSync("readme-template3.md", "utf8");
    ...
    fetch(
        `${xboard_api_url}/fetch`,
        {
            ...
            headers: {
                ...
                'User-Agent': 'Github-Action-v1-gm003',
        ...
        .then((data) => {
            data.data.forEach((item) => {
                subscriptions.push({
                    url:
                        "https://fn10.sp1230.top/api/v1/client/subscribe?token=" +
                        item.token,
                });
            });
            ...
            fs.writeFileSync("dest3/README.md", markdownContent, "utf8");
